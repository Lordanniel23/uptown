...
<div>
   <div class="dropdown">
         <a href="/admin/menu" class="createmenu btn btn-gray-800 d-inline-flex align-items-center me-2" style="background-color: #f0bc74!important;border-color: #f0bc74!important;color:black!important">Menu List</a>
   </div>
</div>
...
<thead>
            ...
            @foreach($category as $categ)
               @if($menu->Category_ID == $categ->id)
                     <td><center>{{$categ->name}}</center></td>
               @endif
            @endforeach
						<td>
						<center>
								<a href="#deleteEmployeeModal" ><button type="button"  class="restore btn btn-success" data-bs-toggle="modal" data-bs-target="#restore" data-id="{{$menu->id}}" id="{{$menu->Name}}">Restore</button></a>
							</center>
						</td>
					</tr> 
                @endif
...
<div class="modal fade" id="restore" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <p>  Are you sure you want to Restore <u><span class="namemenu" style="font-weight:bold"></span></u>  ?</p>
...
$(window).on('load', function() {
  ...
  $('#menuarchul').attr('class', "nav-item active");
  $('#menu-pages').attr('class', "multi-level collapse show");
  $('#menulistspan').attr('aria-expanded', "true");
  document.getElementById("bread").innerHTML += "<li class='breadcrumb-item active'>Menus</li><li class='breadcrumb-item active'>Menu</li><li class='breadcrumb-item active' aria-current='page'>Archived Menu</li>";
  document.getElementById("page-name").innerHTML += "Archived Menu";
	console.log("works");
});
